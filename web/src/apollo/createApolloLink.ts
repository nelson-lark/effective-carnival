...
const createApolloLink = (session: CognitoUserSession): ApolloLink => {
  const config: UrlInfo = {
    url: process.env.REACT_APP_EFFECTIVE_CARNIVAL_AWS_APPSYNC_URL!,
    region: process.env.REACT_APP_EFFECTIVE_CARNIVAL_AWS_APPSYNC_REGION!,
    auth: {
  ...
  const httpLink = new HttpLink({
    uri: process.env.REACT_APP_EFFECTIVE_CARNIVAL_AWS_APPSYNC_URL!,
    fetch: customFetch,
